<?php
	...
	if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true){
		header("location: loginrezENG.php");
		exit;
	}
?>
...
<center><div style="background:rgba(0,0,0,0.6); border-radius:25px; width:250px; height:450px; color:white;">
	<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method ="post">
		<div style="margin-top:70px; font-size:25px;">
		  <div>
				Username:<input type="text" name="username" placeholder="username" autocomplete="off">
		  </div>
		  <br>
		  <div class="category_div" id="category_div">Accommodation:
				<select id="source" name="source" onchange="javascript: dynamicdropdown(this.options[this.selectedIndex].value);">
				<option value="">Choose accommodation</option>
				<option value="Velika soba">Large room</option>
				<option value="Mala soba">Small room</option>
				<option value="Apartman">An apartment</option>
				</select>
			</div>
			<br>
			<div class="sub_category_div" id="sub_category_div">Guest num.:
				<script type="text/javascript" language="JavaScript">
				document.write('<select name="status" id="status"><option value="">Guest num.</option></select>')
				</script>
				<noscript>
			...
			<div style="font-size:25px;">
				...
				Date arrival:<input type="date" id="date1" name="dolazak"><br/>
				<br>
				Date of departure:<input type="date" id="date2" name="odlazak"><br/>
				<br>
				<input type="submit" value="Send" style="width:90px; height:40px; font-size:15px; font-weight:bold;">
				<input type="button" onclick=" window.location.href='hehehe_loggedENG.html';" value="Back" style="width:90px; height:40px; font-size:15px; font-weight:bold;"/>
			</div>
		</div>
	</form>
</div></center>
				...
				if(mysqli_stmt_execute($stmt)){
					...
					if($num_rows > 0){
						...
						if ($result->num_rows > 0) {
							echo '<div style="background:rgba(255,0,0,0.8); border-radius:15px; width:420px; color:white; font-size:18px;">';
							echo "<b>Your dates for the selected accommodation are already taken.<br> Please select new dates that are not among the following:</b>";
							echo "<br /><br />";
							echo"<table >";
							// Printanje rezultata
							while($row = $result->fetch_assoc()) {
								echo "<tr>";
								echo "<td>" ."Date of arrival:".$row["Datum_dolaska"]. "</td>";
								echo "<td>"."</td>";
								echo "<td>" . "Departure date:".$row["Datum_odlaska"]. "</td>";
								echo "</tr>";
							}
							echo"</table>";
							echo'</div>';
		...
		if(empty($username_err) && empty($smjestaj_err) && empty($br_gostiju_err) && empty($datum_dolaska_err) && empty($datum_odlaska_err)){
			...
			if($stmt = mysqli_prepare($link, $sql)){
				...
				if(mysqli_stmt_execute($stmt)){
					...
					//header("location: rezervacijaENG.php");
				}else{
